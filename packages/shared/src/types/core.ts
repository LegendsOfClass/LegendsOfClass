...
export type Targeting = 'lowestHpEnemy' | 'self' | 'lowestHpAlly';
...
export interface SkillDef {
  ...
  targeting: Targeting;
  castCondition: string;     // 'always' | 'selfHp<=N' | 'targets>=N'
}

export interface PrimaryStats { str: number; dex: number; con: number; int: number; }

export interface GearBonus { patk?: number; matk?: number; def?: number; hp?: number; spd?: number; }
